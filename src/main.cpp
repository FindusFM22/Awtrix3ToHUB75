...
{
#ifdef DISPLAY_HUB75
  // HUB75 OE is active-low. Before the DMA driver takes over, drive it HIGH
  // so the panel stays blanked during the ~2-3s boot phase — otherwise the
  // shift-registers hold random data at full brightness and the panel can
  // pull >1A of inrush current, browning out weaker supplies.
  pinMode(15, OUTPUT);
  digitalWrite(15, HIGH);
#else
  // Ulanzi/awtrix2_upgrade: holding GPIO15 low keeps the WS2812 strip blanked
  // until the rest of setup() is ready. On the HUB75 build GPIO15 is OE, so
  // forcing it low here would simply leave the panel permanently enabled
  // (active-low signal). The HUB75 driver claims and toggles OE itself.
  pinMode(15, OUTPUT);
  digitalWrite(15, LOW);
#endif
  delay(2000);
