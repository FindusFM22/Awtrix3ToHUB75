...
#elif defined(DISPLAY_HUB75)
// No single data pin — HUB75 uses 13 parallel lines, configured in setup().
#else
...
// Physical panel: 64x32. Logical canvas: 32x8 (same as Ulanzi) blitted
// centred onto the panel. Apps/effects/notifications remain unchanged.
#define HUB75_PANEL_W 64
#define HUB75_PANEL_H 32
#define HUB75_PANEL_CHAIN 1
#define HUB75_OFFSET_X ((HUB75_PANEL_W - MATRIX_WIDTH) / 2)   // = 16
#define HUB75_OFFSET_Y ((HUB75_PANEL_H - MATRIX_HEIGHT) / 2)  // = 12
...
// HUB75 path treats leds[] as a logical shadow buffer and blits to the panel.
// All internal call sites use this instead of matrix->show() directly.
...
void DisplayManager_::setBrightness(int bri)
{
  ...
  if (dma_display)
  {
    dma_display->setBrightness8(static_cast<uint8_t>(bri));
    // HUB75 BCM does not blank on brightness=0 (OE keeps scanning);
    // clear the DMA framebuffer to make the panel actually go dark.
    if (bri == 0)
...
void DisplayManager_::setMatrixLayout(int layout)
{
  ...
  case 2:
    ...
    break;
  default:
    // Fallback for stored values outside the supported 0..2 range. Without
    // this we would leave `matrix` dangling after the `delete` above and
    // hand a freed pointer to the new MatrixDisplayUi. Mirror layout 1
    // (the previous default global matrix in this file).
    matrix = new FastLED_NeoMatrix(leds, 8, 8, 4, 1, NEO_MATRIX_TOP + NEO_MATRIX_LEFT + NEO_MATRIX_ROWS + NEO_MATRIX_PROGRESSIVE);
    break;
  }
...
void DisplayManager_::blitToPanel()
{
  ...
  const int oy = (HUB75_PANEL_H - h) / 2;
  const bool haveCorr = COLOR_CORRECTION.r || COLOR_CORRECTION.g || COLOR_CORRECTION.b;
  const bool haveTemp = COLOR_TEMPERATURE.r || COLOR_TEMPERATURE.g || COLOR_TEMPERATURE.b;
  const CRGB corr = haveCorr ? COLOR_CORRECTION : CRGB(0xFFFFFF);
  const CRGB temp = haveTemp ? COLOR_TEMPERATURE : CRGB(0xFFFFFF);
  for (int y = 0; y < h; y++)
  {
    for (int x = 0; x < w; x++)
    {
      CRGB c = leds[matrix->XY(x, y)];
      uint8_t r = ((uint16_t)c.r * corr.r / 255 * temp.r) / 255;
      uint8_t g = ((uint16_t)c.g * corr.g / 255 * temp.g) / 255;
      uint8_t b = ((uint16_t)c.b * corr.b / 255 * temp.b) / 255;
